...
def arg_treatment():
    ...
    if ARGV['output'] == '':
        ARGV['output'] = re.sub(
            r"^(?P<name>(?:(?!\.md$).)+)(?:\.md$)?", r"\g<name>.html", ARGV['input'])
...
def parse(paragraph):
    ...
    paragraph = re.sub(
        r"(?<!\w)\"(?=\w)(?P<quote>.*?)(?<=\w)\"(?!\w)", r"<div class='quote'>\g<quote></div>", paragraph)
    paragraph = re.sub(
        r"(?<!\w)'(?=\w)(?P<quote>.*?)(?<=\w)'(?!\w)", r"<div class='quote'>\g<quote></div>", paragraph)
    ...
    for i in range(len(fragments)):
        if fragments[i] != '' and fragments[i][0] != '$' and fragments[i][0:min(len(fragments[i]), 2)] != "\\[":
            ...
            fragments[i] = re.sub(
                r"\<(?P<link>https?://[^ ]*)\>", "<a href='\g<link>'>\g<link></a>", fragments[i])
            # Links like " http://www.google.com "
            fragments[i] = re.sub(
                r" (?P<link>https?://[^ ]*) ", " <a href='\g<link>'>\g<link></a> ", fragments[i])
...
def main():
    ...
    print("HTML output file written in :", outFile)
